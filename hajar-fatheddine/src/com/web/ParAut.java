 ...
 */
@WebServlet({ "/ParAut"  })
public class ParAut extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.print("<form action='serveParAut' method='post'> <table>");
		...
		//recupere auteur envoye par formulaire et retournee ci-dessous
		String auteur = (String) request.getAttribute("auteur");
				
		List<com.model.Livre> ls= lm.allLivreByAuteur(auteur);
		...
		for (com.model.Livre l : ls) {
			if (l.getQuantite()!=0) { //lister si la quntite d un livre est !=0
				...
				out.print("<td><a href='serveParAut?op=reserver&id="+l.getId()+"'>reserver</a></td>");
				out.print("</tr>");
			}
		}
		out.print("</tbody>");
